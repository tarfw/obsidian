...
| Related proposal | [site.md](site.md): public customer projection of a workspace |
...
| Site | A workspace's public customer projection |

**Key rule:** context changes relevance. It never grants permission.

Space serves authorized internal work. [Site](site.md) serves customers through
the same Commerce Core, Gateway, records, and Flow Books.

## 2. A day in Space

> **Illustrative scenario only.** These times, jobs, workspaces, roles, and
> cards are a test example. Production Space is open to any permitted personal,
> business, community, learning, service, or other context. Nothing below is a
> hardcoded schedule or required workspace sequence.
...
Restaurant is one example of the shared pattern for all commerce. The request
may begin on a [Site](site.md) or in Ask TAR and creates the same canonical work.
...
## 9. TAR execution architecture

**Kernel** means the small foundation everything relies on. In the product,
call it the **Commerce Core**: authoritative orders, inventory, payments,
books, identities, permissions, and exact business rules.
...
~~~text
          ...
          +-----------------+-----------------+
          |                 |                 |
      UI / Site        Flow Runner       Agent Control
                       Flow Books       OpenAI + JEV
          |                 |                 |
          +-----------------+-----------------+
                            |
                            v
         ...
         |                 |                  |
         v                 v                  v
   Commerce Core        API / MCP       Sandbox / Browser
   orders, stock,       external        exceptional work
   payments, books      systems
~~~

Agent Control does not sit inside the Gateway. It reasons, plans, and proposes
actions. Direct UI, Flow Books, and agents all use the same Gateway for business
effects. Read-only reasoning needs no Gateway call.

| Layer | Owns |
|---|---|
| Commerce Core | Business truth, calculations, rules, and state |
| Agent Control | Reasoning session, JEV judgments, planning, environment choice |
| Flow Runner | Durable coordination across actions, time, events, and people |
| Action Gateway | Access, validation, policy, replay, approval, and receipts |
| Site Runtime | Public pages and commerce actions from the same workspace truth |
| Executors | Native code, API/MCP, sandbox, browser, or computer use |

### Space composition path
...
| Process | Domain state and dependencies; Flow Books coordinate its actions |
...
## 10. Durable Flow Books

~~~text
Flow Book = published reusable recipe
Flow run  = one durable execution of that recipe
~~~

A Flow Book references registered Gateway actions. It may branch, run work in
parallel, wait for a person or event, retry, recover, and call an agent when a
bounded part cannot be planned in advance.

### How a Flow Book is made

~~~text
describe the process
        |
        v
JEV matches eligible actions and conditions
        |
        v
code creates typed draft
        |
        v
preview + sample test + grants + cost
        |
        v
publish immutable edition
~~~

Editing a published book creates a new edition. Existing runs remain pinned to
the edition and inputs with which they started.

### Shipped flow: order to delivery

~~~text
Order created
     |
     v
check payment + reserve stock
     |
     v
restaurant accepts
     |
     ...
     +--> drinks action
        ...
        wait for both
             |
             v
pack -> assign courier -> deliver
             |
             v
complete order -> post books -> notify customer
~~~

### Workspace-created flow

Example: a sandwich shop creates a nightly replenishment book.

~~~text
22:30 closing
     |
     v
count critical ingredients
     |
     v
JEV matches unclear supplier item names
     |
     v
prepare purchase order
     |
     v
owner approval in Inbox
     |
     v
send order -> wait for delivery -> receive stock
~~~

The workspace composes existing stock, purchasing, approval, notification, and
supplier actions. It cannot redefine accounting, permissions, or stock rules.

### What ships and what can be created

| Area | Commerce Core ships | Starter Flow Book | Workspace may create |
|---|---|---|---|
| Orders | Order, payment, state and access rules | Order to fulfillment | Catering or wholesale routing |
| Inventory | Items, reservations, movements and counts | Low stock to purchase | Recipe or branch replenishment |
| Purchasing | Supplier, purchase order and receiving | Purchase to stock | Supplier-specific approval |
| POS | Sale, register, receipt and refund | Sale to closing | Shift or location closing |
| Books | Ledger, invoice, expense and posting rules | Sale to posting | Review and reconciliation schedule |
| Returns | Return, inspection and refund rules | Return to refund | Category-specific inspection |
| Delivery | Assignment, handoff and proof | Pickup to delivery | Zone or fleet routing |
| Contacts | People, organizations and relationships | Enquiry to quote | Workspace-specific follow-up |

Core commerce remains complete when a user performs actions directly. Starter
Flow Books provide common automation; custom books coordinate the same trusted
actions for a particular business.

### What makes a run durable

~~~text
execute one Gateway action
          |
          v
save checkpoint + receipt
     ...
     +----+--------------------+
     |                         |
 next step               wait / failure
     |                         |
     v                         v
continue          resume event / retry / recover
~~~

| Saved item | Purpose |
|---|---|
| Edition and input | Reproduce the exact intended recipe |
| Cursor and state | Know the next unfinished step |
| Idempotency key | Prevent duplicate payment, order, or message |
| Receipt | Prove the confirmed result |
| Wait | Resume after time, webhook, human action, or record change |
| Attempt and error | Apply bounded retry or recovery |
| Actor and grants | Recheck current authority before consequential work |
| Budget | Stop or ask before exceeding the allowed cost |

App closure, server restart, agent loss, or sandbox shutdown does not erase the
run. The durable runner reloads the checkpoint and continues from the first
unfinished safe step.

## 11. Example JEV cost per person
...
| Day example | Illustrative acceptance scenario; no time, job, role, or sequence is hardcoded |
...
| Site | Public projection compiled from [site.md](site.md) and live workspace truth |
| Commerce Core | Authoritative orders, stock, payments, books, access, and rules |
| Action Gateway | One effect boundary shared by UI, Flow Books, and agents |
| Flow Books | Versioned recipes that coordinate registered actions |
| Durability | Pinned edition, checkpoints, receipts, waits, replay, and recovery |
| Custom flows | Compose trusted actions; cannot redefine core business invariants |
