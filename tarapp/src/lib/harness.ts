...
export const harness = {
  ...
  inviteMember: (slug: string, email: string, role: Exclude<HarnessRole, 'owner'> = 'member') => request<{ invitation: { email: string; role: Exclude<HarnessRole, 'owner'>; state: 'pending' } }>(workspacePath(slug, 'members'), { method: 'POST', body: { email, role }, key: createOperationKey(`invite:${slug}:${email}`) }),
